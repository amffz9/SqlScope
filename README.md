...
An IntelliJ/PhpStorm plugin that lets you assign SQL dialects and data source resolution scopes to project directories from a right-click context menu — no more digging through Settings → Languages & Frameworks → SQL Dialects.

## The Problem

When working on projects that mix SQL across multiple directories (e.g. `src/migrations/` using MySQL and `src/reporting/` using PostgreSQL), setting the correct dialect and resolution scope for each directory requires several clicks through the IDE settings panel. SQLScope puts those mappings directly in the Project View context menu.

## Features

- **Set SQL Dialect** — right-click any directory and assign MySQL, PostgreSQL, SQLite, MariaDB, Oracle SQL*Plus, SQL Server (T-SQL), or Generic SQL
...
- **Set Resolution Scope** — associate a directory with a specific datasource or schema for accurate SQL symbol resolution and autocomplete
...
```
Right-click a directory in Project View
└── SQLScope
    ├── ── Dialect ────────────────────
...
```bash
./gradlew buildPlugin      # → build/distributions/SQLScope-1.0.0.zip
